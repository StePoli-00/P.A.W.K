...
real_height = 1.7  # Altezza media della persona in metri
...
with mp_pose.Pose(static_image_mode=True, min_detection_confidence=0.5) as pose:
    for index, person in person_detections.iterrows():
        ...
        if results.pose_landmarks:
            ...
            put_text_on_image(image, label, (xmin, ymin - 10), font_scale=font_scale, thickness=1)
        
        text_position = (xmin, ymin - 20)
        if text_position[1] < 0:  
            text_position = (xmin, ymin + 20)
                
        put_text_on_image(image, f'Distance: {distance:.2f}m', text_position, font_scale=font_scale, thickness=1)        
