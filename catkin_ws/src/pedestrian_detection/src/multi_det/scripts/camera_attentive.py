...
def is_facing_camera(torso_vector, shoulder_distance, torso_height, threshold=0.9, distance_ratio_threshold=0.6):
    ...
    if cos_theta > threshold:
        distance_ratio_threshold *= (torso_height / 0.7293796125061089)  # Adjust ratio threshold similarly
        if shoulder_distance / torso_height > distance_ratio_threshold:
            return True
...
while cap.isOpened():
    ...
    if results.face_landmarks:
        ...
        attentive = "attention_level = 0.1" 
        # Body orientation detection
        if results.pose_landmarks:
            torso_vector, shoulder_distance, torso_height = calculate_torso_vector(results.pose_landmarks)
              
            if is_facing_camera(torso_vector, shoulder_distance, torso_height):
                label = "Body facing camera"
                if face_text == "Looking Forward":
                    attentive = "attention_level = 1"
                    label = "Body and face facing camera"
                else: 
                    label = "Body facing camera, face not facing camera"
                    attentive = "attention_level = 0.5"
            else:
                if face_text == "Looking Forward":
                    label = "Body not facing camera, face facing camera"
                    attentive = "attention_level = 0.5"
                else:
                    label = "Body and face not facing camera"
                    attentive = "attention_level = 0.1" 
        ...
        cv2.putText(image, face_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    ...
    cv2.putText(image, label, (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

    #cv2.putText(image, f'FPS: {int(fps)}', (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    cv2.putText(image, attentive, (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
