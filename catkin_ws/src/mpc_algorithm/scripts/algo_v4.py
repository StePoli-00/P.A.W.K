#!/usr/bin/python3

import rospy
import numpy as np
from geometry_msgs.msg import Twist, PoseStamped, PoseWithCovarianceStamped
from sensor_msgs.msg import LaserScan
from tf.transformations import euler_from_quaternion, quaternion_from_euler
from copy import deepcopy

current_pose = None
rear_laser_scan = None

omega_goal = 1.0
omega_obstacle = 0.9

angle_min = -2.3561999797821045
angle_max = 2.3561999797821045
angle_increment = 0.008726666681468487


def amcl_pose_callback(msg):
    global current_pose
    current_pose = msg.pose.pose


def rear_laser_scan_callback(msg):
    global rear_laser_scan
    rear_laser_scan = msg.ranges


def calculate_cost(
    current_pose, goal_pose, obstacles, omega_goal, omega_obstacle, beta
):

    distance_to_goal = np.linalg.norm(
        np.array([current_pose.position.x, current_pose.position.y])
        - np.array([goal_pose.pose.position.x, goal_pose.pose.position.y])
    )

    phi_goal = distance_to_goal**2

    phi_obs_dynamic = 0
    w3 = 1 / (beta + 0.1)
    for dyn_obs in obstacles:
        distance_dyn_obs = np.linalg.norm(
            np.array([current_pose.position.x, current_pose.position.y]) - dyn_obs
        )
        if distance_dyn_obs < 2:
            phi_obs_dynamic += w3 * (1 / (distance_dyn_obs**2))

    cost = omega_goal * phi_goal + omega_obstacle * phi_obs_dynamic
    return cost


def get_obstacle_positions(front_scan):
    obstacles = []
    num_readings_front = len(front_scan)

    for i in range(num_readings_front):
        r = front_scan[i]
        if r < 2:
            angle = angle_min + i * angle_increment
            x = r * np.cos(angle)
            y = r * np.sin(angle)
            obstacles.append([x, y])
    return obstacles


def mpc_control_loop(goal_pose, beta):
    rate = rospy.Rate(20)
    delta_t = 1 / 20

    while not rospy.is_shutdown():
        if current_pose is None or rear_laser_scan is None:
            rospy.loginfo("In attesa dei dati dei sensori...")
            rate.sleep()
            continue

        obstacles = get_obstacle_positions(rear_laser_scan)

        if obstacles:
            min_distance_to_obstacle = min(
                [
                    np.linalg.norm(
                        np.array([current_pose.position.x, current_pose.position.y])
                        - np.array(obs)
                    )
                    for obs in obstacles
                ]
            )
            adjusted_omega_obstacle = min(
                omega_obstacle * (1 / (min_distance_to_obstacle)), 2
            )
            rospy.loginfo(f"OMEGA OBSTACLE ADJUSTED: {adjusted_omega_obstacle}")
        else:
            adjusted_omega_obstacle = omega_obstacle

        best_cost = float("inf")
        best_twist = Twist()

        for linear_vel in np.linspace(1, -0.5, 5):
            for angular_vel in np.linspace(-1, 1, 10):

                simulated_pose = deepcopy(current_pose)
                current_orientation = simulated_pose.orientation
                _, _, current_theta = euler_from_quaternion(
                    [
                        current_orientation.x,
                        current_orientation.y,
                        current_orientation.z,
                        current_orientation.w,
                    ]
                )

                simulated_pose.position.x += (
                    linear_vel * np.cos(current_theta) * delta_t
                )
                simulated_pose.position.y += (
                    linear_vel * np.sin(current_theta) * delta_t
                )
                new_theta = current_theta + angular_vel * delta_t
                simulated_pose.orientation = quaternion_from_euler(0, 0, new_theta)

                cost = calculate_cost(
                    simulated_pose,
                    goal_pose,
                    obstacles,
                    omega_goal,
                    omega_obstacle,
                    beta,
                )

                if cost < best_cost:
                    best_cost = cost
                    best_twist.linear.x = linear_vel
                    best_twist.angular.z = angular_vel

        goal_angle = np.arctan2(
            goal_pose.pose.position.y - current_pose.position.y,
            goal_pose.pose.position.x - current_pose.position.x,
        )
        _, _, current_theta = euler_from_quaternion(
            [
                current_pose.orientation.x,
                current_pose.orientation.y,
                current_pose.orientation.z,
                current_pose.orientation.w,
            ]
        )
        angle_diff = goal_angle - current_theta
        angle_diff = np.arctan2(np.sin(angle_diff), np.cos(angle_diff))

        best_twist.angular.z = angle_diff * adjusted_omega_obstacle
        cmd_vel_pub.publish(best_twist)

        distance_to_goal = np.linalg.norm(
            np.array([current_pose.position.x, current_pose.position.y])
            - np.array([goal_pose.pose.position.x, goal_pose.pose.position.y])
        )
        rospy.loginfo(f"Distanza dal target: {distance_to_goal:.2f}")
        rospy.loginfo(
            f"Miglior comando: Velocità lineare: {best_twist.linear.x}, Velocità angolare: {best_twist.angular.z}"
        )

        rate.sleep()


if __name__ == "__main__":
    rospy.init_node("mpc_algorithm")

    cmd_vel_pub = rospy.Publisher("/robot/cmd_vel", Twist, queue_size=10)

    rospy.Subscriber("/robot/amcl_pose", PoseWithCovarianceStamped, amcl_pose_callback)
    rospy.Subscriber("/robot/rear_laser/scan", LaserScan, rear_laser_scan_callback)

    goal_pose = PoseStamped()
    goal_pose.pose.position.x = -2.0
    goal_pose.pose.position.y = 0.0
    beta = 1

    mpc_control_loop(goal_pose, beta)
